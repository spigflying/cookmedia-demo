/* 庫客行銷 Cookmedia — 改版 Demo 動畫(淺色版)
 * 使用 GSAP + ScrollTrigger。動畫邏輯與深色版一致,
 * 僅背景粒子顏色針對淺底重新調校。 */
...
(function particles() {
  ...
  function draw() {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      ctx.fillStyle = "rgba(47,107,255,0.45)";
      ctx.fill();
      ...
      for (let j = i + 1; j < pts.length; j++) {
        ...
        if (dist < 120) {
          ...
          ctx.moveTo(p.x, p.y);
          ctx.lineTo(q.x, q.y);
          ctx.strokeStyle = `rgba(123,60,255,${0.16 * (1 - dist / 120)})`;
          ctx.stroke();
        }
      }
    }
